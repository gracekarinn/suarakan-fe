...
const [selectedInstitution, setSelectedInstitution] = useState<ReportingLevel>();
const [selectedStatus, setSelectedStatus] = useState<CaseStatus>(report.status || CaseStatus.Reported);
...
return (
    ...
    <h3 className="text-lg font-semibold text-[#8B322C] mb-4">Update Status Laporan</h3>
    
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
        <label className="block text-sm font-medium text-gray-700">Laporkan ke:</label>
        <div className="flex gap-2">
            <select
            value={selectedInstitution || ''}
            onChange={(e) => setSelectedInstitution(e.target.value as ReportingLevel)}
        ...
        <div className="mt-2 text-sm text-gray-500">
            Status saat ini: <span className="font-medium">{report.status}</span>
        </div>
        </div>
    </div>
    </div>
